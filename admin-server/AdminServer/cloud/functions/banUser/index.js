...
// 原始处理函数
async function banUserHandler(event, context) {
        ...
        await collection.doc(user._id)
            .update({
                data: {
                    ...
                    banUntil: banUntil,
                    banBy: event.adminInfo.username, // 记录封禁操作者
                    gmtModify: moment().format("YYYY-MM-DD HH:mm:ss")
                }
            });

        // 记录操作日志
        await logOperation(event.adminInfo, 'BAN', 'USER', {
            appId: appId,
            playerId: playerId,
            reason: reason,
            duration: duration,
...
// 导出带权限校验的函数
const mainFunc = requirePermission(banUserHandler, 'user_manage');
