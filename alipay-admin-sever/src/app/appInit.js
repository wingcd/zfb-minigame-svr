...
async function appInitHandler(event, context) {
    ...
    if(event.hasOwnProperty("channelAppId") && (!event.channelAppId || typeof event.channelAppId != "string")) {
        ret.code = 4001;
        ret.msg = "参数[channelAppId]错误";
        return ret;
    }

    if(event.hasOwnProperty("channelAppKey") && (!event.channelAppKey || typeof event.channelAppKey != "string")) {
        ret.code = 4001;
