...
type AdminOperationLog struct {
	...
	IpAddress string `orm:"size(45);column(ip_address)" json:"ipAddress"`
	UserAgent string `orm:"size(500);column(user_agent)" json:"userAgent"`
}
...
func GetOperationsByDate(date string) (int64, error) {
	...
	var count int64
	// 使用DATE函数来比较日期
	err := o.Raw("SELECT COUNT(*) FROM admin_operation_logs WHERE DATE(created_at) = ?", date).QueryRow(&count)
	return count, err
}
