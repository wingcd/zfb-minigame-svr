...
type AdminOperationLog struct {
	...
	IpAddress string `orm:"size(45);column(ipAddress)" json:"ipAddress"`
	UserAgent string `orm:"size(500);column(userAgent)" json:"userAgent"`
}
...
func GetOperationLogList(page, pageSize int, username, action string, startTime, endTime time.Time) ([]AdminOperationLog, int64, error) {
	...
	if !startTime.IsZero() {
		qs = qs.Filter("createdAt__gte", startTime)
	}
	if !endTime.IsZero() {
		qs = qs.Filter("createdAt__lte", endTime)
	}
	...
	_, err := qs.OrderBy("-createdAt").Limit(pageSize, offset).All(&logs)
...
func GetOperationLogStats(startTime, endTime time.Time) (map[string]interface{}, error) {
	...
	totalCount, err := o.QueryTable("admin_operation_logs").
		Filter("createdAt__gte", startTime).
		Filter("createdAt__lte", endTime).
		Count()
	if err != nil {
		return nil, err
	}
	...
	_, err = o.Raw("SELECT action as module, COUNT(*) as count FROM admin_operation_logs WHERE createdAt BETWEEN ? AND ? GROUP BY action ORDER BY count DESC", startTime, endTime).Values(&moduleStats)
	if err != nil {
		return nil, err
	}
	...
	_, err = o.Raw("SELECT username, COUNT(*) as count FROM admin_operation_logs WHERE createdAt BETWEEN ? AND ? GROUP BY username ORDER BY count DESC LIMIT 10", startTime, endTime).Values(&adminStats)
	if err != nil {
		return nil, err
	}
...
func GetOperationsByDate(date string) (int64, error) {
	...
	var count int64
	// 使用DATE函数来比较日期
	err := o.Raw("SELECT COUNT(*) FROM admin_operation_logs WHERE DATE(createdAt) = ?", date).QueryRow(&count)
	return count, err
}
