...
/**
 * 根据game-service中间件生成API签名
 * @param {object} params - 请求参数对象
 * @param {number} timestamp - 时间戳
 * @param {string} appSecret - 应用密钥
 * @returns {string} MD5签名
 */
function generateApiSign(params, timestamp, appSecret) {
    // 将参数按键名排序
    ...
    // 添加时间戳和密钥
    let signStr = signParts.join('&');
    if (signStr !== '') {
        signStr += '&';
    }
    signStr += `timestamp=${timestamp}&key=${appSecret}`;
